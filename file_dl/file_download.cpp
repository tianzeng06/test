...
int Http_link_parse(char *link, char **url, char **port, char **path, char **file_name){
	...
	//http://www-us.apache.org/dist/hbase/stable/hbase-1.2.4-bin.tar.gz
     url_begin=strstr(link,"http://");//url_begin=='h'
     if(NULL==url_begin)
	 {
     ...
     url_begin=url_begin+7;//url_begin=='w'
	 ...
	 {
        port_length=&(link[i])-colon-1;//-1是除去:的位置 
        port_begin=colon+1;
        url_length=colon-url_begin;
     }

     char *file_name_tmp=&(link[i])+1;//i指向'/' 
...
int Http_get_ip_str_from_url(char *url, char **ip_str)
{
     ...
     */
     ...
     if(NULL!=addrinfo_cur)
	 {
        ...
        const char *ret=inet_ntop(AF_INET,&(sockin->sin_addr),ip_addr_str,INET_ADDRSTRLEN);//把二进制ip转化为点分十进制 
...
int Http_connect_to_server(char *ip, int port, int *socket_fd)
{
     ...
     *socket_fd=socket(AF_INET,SOCK_STREAM,0);//tcp链接 
     if(*socket_fd<0)
	 {
     ...
     int ret_0 =inet_pton(AF_INET,ip,&(sock_address.sin_addr.s_addr));//转化为网络字节序 
     if(ret_0!=1)
	 {
...
int JHttp_download_whole_file(char *link)
{
     ...
     */
     if(NULL==link)
	 {
        printf("Http_download_whole_file: argument error\n");
        exit(0);
     }
